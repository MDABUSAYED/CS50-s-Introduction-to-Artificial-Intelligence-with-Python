...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        neighbor = {(i, j) for j in range(cell[1] - 1, cell[1] + 2) for i in range(cell[0] - 1, cell[0] + 2) if 0 <= i < self.height and 0 <= j < self.width and (i, j) != cell}

        new_sentence = Sentence(neighbor, count)
        ...
        for sentence in self.knowledge:
            new_mines = {cell for cell in sentence.known_mines()}
            new_safes = {cell for cell in sentence.known_safes()}
        ...
        for sentence1, sentence2 in itertools.combinations(self.knowledge, 2):
            ...
            if inferred_sentence is not None and inferred_sentence not in self.knowledge:
    ...
    def make_safe_move(self):
        ...
        return random.choice(tuple(moves)) if moves else None
    ...
    def make_random_move(self):
        ...
        moves = set(itertools.product(range(0, self.height), range(0, self.width)))
        moves = moves - self.mines - self.moves_made
