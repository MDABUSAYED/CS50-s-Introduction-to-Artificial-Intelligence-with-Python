...
class NimAI():
    ...
    def best_future_reward(self, state):
        ...
        for action in Nim.available_actions(list(state)):
            best_reward = max(self.get_q_value(state, action), max_reward)
    ...
    def choose_action(self, state, epsilon=True):
        ...
        for action in actions:
            if max_action is None or self.get_q_value(state, action) > max_reward:
                max_reward = self.get_q_value(state, action)
                max_action = action

        if epsilon:
            weights = [(1 - self.epsilon) if action == max_action else (self.epsilon / (len(actions) - 1)) for action in actions]
